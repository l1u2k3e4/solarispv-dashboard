    ...
    <header className="sticky top-0 z-40 w-full border-b border-border bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80">
      <div className="container-wide flex h-16 items-center justify-between gap-4 lg:h-20">
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Hauptnavigation">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-md px-3 py-2 text-sm font-bold text-sternhoff-text-dark transition-colors hover:text-sternhoff-primary focus:outline-none focus-visible:ring-2 focus-visible:ring-sternhoff-primary focus-visible:ring-offset-2"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-2">
          <Button asChild variant="accent" size="default" className="hidden md:inline-flex">
          ...
          <a
            ...
            aria-label="Anrufen"
            className="inline-flex h-11 w-11 items-center justify-center rounded-md bg-sternhoff-accent text-white md:hidden"
          >
          ...
          <Sheet open={open} onOpenChange={setOpen}>
            <SheetTrigger asChild>
              <button
                type="button"
                className="inline-flex h-11 w-11 items-center justify-center rounded-md border border-border text-sternhoff-text-dark transition-colors hover:bg-sternhoff-bg-light lg:hidden"
              ...
              <nav className="flex flex-col gap-1" aria-label="Mobile Navigation">
                {navItems.map((item) => (
                  <SheetClose asChild key={item.href}>
                    <Link
                      href={item.href}
                      className="rounded-md px-3 py-3 text-base font-bold text-sternhoff-text-dark transition-colors hover:bg-sternhoff-bg-light"
                    >
                      {item.label}
                    </Link>
                  </SheetClose>
